function FingerCurlDebugPanel({ hands }) {
  ...
  return (
    <aside className="ae-debug-panel" aria-label="Development finger tap diagnostics">
      <p className="ae-debug-panel__title">DEV · Finger tap diagnostics</p>
      <p className="ae-debug-panel__legend">motion delta / on / off · thumb uses arc motion · N/A = no key assigned</p>
      {hands.map((hand) => (
        <section key={hand.id} className="ae-debug-panel__hand" style={{ '--ae-debug-color': hand.color ?? '#7C6FFF' }}>
          ...
          {hand.fingers.map((finger) => (
            <div key={finger.name} className="ae-debug-panel__row">
              <span>{finger.name}{finger.motionAxis === 'arc' ? ' · arc' : ''}{finger.note ? ` · ${finger.note}` : ''}</span>
